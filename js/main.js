...
function createProjectCard(project, isFeatured) {
  ...
  // Live Demo button at the bottom of the card (only if liveDemo exists)
  const liveDemoButton = project.liveDemo 
    ? `<a href="${project.liveDemo}" target="_blank" rel="noopener" class="btn-live-demo">
  ...
  return `
    <article class="project-card ${isFeatured ? 'featured' : ''}" data-tags="${project.tags.join(',').toLowerCase()}">
      ...
      ${liveDemoButton ? `<div class="project-footer">${liveDemoButton}</div>` : ''}
    </article>
  `;
}
